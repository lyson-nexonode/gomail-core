...
func main() {
	...
	telemetry.StartPPROF("localhost:6063", log)

	// Graceful shutdown on SIGINT or SIGTERM
	ctx, cancel := signal.NotifyContext(context.Background(), os.Interrupt, syscall.SIGTERM)
	defer cancel()

	server := jmap.NewServer(cfg, log, ms, ms, ms, ms)
	if err := server.Start(ctx); err != nil {
		log.Fatal("jmap server failed", zap.Error(err))
	}

	log.Info("jmap server stopped")
}
